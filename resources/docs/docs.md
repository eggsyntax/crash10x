...
The `home-routes` handler in the `crash10x.routes.home` namespace
...
The `home-page` function will in turn call the `crash10x.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/crash10x/core.cljs` file.



<p class="title is-5">Organizing the routes</p>

The routes are aggregated and wrapped with middleware in the `crash10x.handler` namespace:
...
Request middleware functions are located under the `crash10x.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `crash10x.dev-middleware` namespace found in
